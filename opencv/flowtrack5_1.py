...
trackPlayer = player.Player(client)
...
class Track:
    ...
    def addPoint(self,pt):
        ...
        if not self.isOn and on:
            ...
            trackPlayer.on(self.pts[-1],(dx,dy),vel,self.key)
 
        elif self.isOn and not on:
            self.isOn=False
            trackPlayer.off(self.key)
            ...
            if self.frame_idx % self.detect_interval == 0:
                ...
                if p is not None:
                    for x, y in np.float32(p).reshape(-1, 2):
                        start=(x, y)
                        self.keyCnt += 1
                        self.tracks.append(Track(self.keyCnt,start))
